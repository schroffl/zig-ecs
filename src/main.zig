...
fn simpleBenchmark(allocator: *Allocator, num_entities: u32, num_systems: u32) !BenchmarkResult {
    var manager = TestManager.init(allocator);
    var system_i: u32 = 0;

    while (system_i < num_systems) : (system_i += 1) {
        var math_system = MathTestSystem.init();
        try manager.addSystem(&math_system.system);
    }
    ...
    while (entity_i < num_entities) : (entity_i += 1) {
        var entity = try manager.spawn();
        _ = entity.add(.Integer);
        try manager.signal(entity);
    }

    var entity_time = timer.lap();
    manager.runSystems();
    var system_time = timer.read();
    ...
    return BenchmarkResult{
        ...
        .combined = system_time + entity_time,
    };
}

test "Very basic benchmark" {
    const Setup = struct {
        entities: u32,
        systems: u32,
    };
    ...
    const setups = []Setup{
        Setup{ .entities = 50, .systems = 1 },
        Setup{ .entities = 500, .systems = 1 },
        Setup{ .entities = 5000, .systems = 1 },
        Setup{ .entities = 50000, .systems = 1 },

        Setup{ .entities = 50, .systems = 50 },
        Setup{ .entities = 500, .systems = 500 },
        Setup{ .entities = 5000, .systems = 500 },
    };
    ...
    for (setups) |setup, i| {
        var benchmark = try simpleBenchmark(allocator, setup.entities, setup.systems);
        std.debug.warn(
            "\t{}. Took {}ns to spawn {} entities ({}ns) and run {} systems ({}ns)\n",
            i + 1,
            benchmark.combined,
            setup.entities,
            benchmark.entity_time,
            setup.systems,
            benchmark.system_time,
        );
    }
